...
switch scenario.number
    case 8 % Offset along x-axis
        interval = [-10*cos(pi/4) 5*cos(pi/4)]; % [meter]
    otherwise
        interval = [];
end
end
...
for k = 1:numel(scenario.sweepVals)  % Move/rotate of surface(s) or antenna (s);
    ...
    for m = 1:numel(scattSurfOrdered)
        ...
        if any(strcmp(models.List, 'erf'))
            gammaErfSingleReflection(k,m)     = ourErfModel(scenario, iteration, m);
        end
        if any(strcmp(models.List, 'METIS'))
            gammaMetisSingleReflection(k,m)   = metisModel(scenario, iteration, m);
        end
        if any(strcmp(models.List, 'METISRCS'))
            gammaMetisRCSSingleReflection(k,m)= metisRCSmodel(scenario, iteration, m);
        end
        if any(strcmp(models.List, 'mmMAGIC'))
            gammaMMMagicSingleReflection(k,m) = mmMAGICModel(scenario, iteration, k, m);
        end
        if any(strcmp(models.List, 'ITUFresnel'))
            gammaITUFresSingleReflection(k,m) = ITUfresnelModel(scenario, iteration, m);
        end
        if any(strcmp(models.List, 'ITUEmpirical'))
            gammaITUEmpSingleReflection(k,m)  = ITUsemiEmpModel(scenario, iteration, k, m);
        end
        ...
        if numel(scattSurfOrdered) > 1
            ...
            if blockage.blockedPath(m)
                gammaFresnel(k,m) = scenario.gamma(m)*gammaFresnelSingleReflection(k,m)*...
                    (1 - gammaFresnelBlockFromTx(k, blockageMatrixFromTx)) * ...
                    (1 - gammaFresnelBlockToRx(k, blockageMatrixToRx));
            else
                gammaFresnel(k,m) = scenario.gamma(m)*gammaFresnelSingleReflection(k,m);
            end
        else
...
models.resultsTh(:, 3) = LOScomponentTh.';
models.resultsPh(:, 3) = LOScomponentPh.';

% Sum the contributions of all interactions.
models.resultsTh(:, 4) = sum(models.resultsTh, 2);
models.resultsPh(:, 4) = sum(models.resultsPh, 2);
end
...
models.List = {'Surf. 1'; 'Surf. 2'; 'LoS'; '3D Fresnel'};
...
for iii = 1:numel(models.List)    
    ...
    if iii == 2
        modelsTikzPhase(xValue<-3, iii) = NaN;
        modelsTikzPhase(round(numel(xValue)*2/3-2):round(numel(xValue)*2/3+3), iii) = ...
            modelsTikzPhase(round(numel(xValue)*2/3-2):round(numel(xValue)*2/3+3), iii) + 360;
    end
